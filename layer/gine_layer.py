...
class GIN_WithEdgeUpdate_Conv(pyg_nn.conv.MessagePassing):
    def __init__(self, emb_dim):
        ...
        self.mlp_e = torch.nn.Sequential(torch.nn.Linear(emb_dim, 2*emb_dim), torch.nn.ReLU(), torch.nn.Linear(2*emb_dim, emb_dim))
        #self.norm = nn.BatchNorm1d(emb_dim)

    def forward(self, batch):
        x, edge_index, edge_attr = batch.x, batch.edge_index, batch.edge_attr
        edge_embedding = self.mlp_e(edge_attr)
        out = self.mlp((1 + self.eps) *x + self.propagate(edge_index, x=x, edge_attr=edge_embedding))
        #batch.x = self.norm(out)
        return batch
...
class GINEConv_v2(pyg_nn.conv.MessagePassing):
    def __init__(self, emb_dim, dropout, residual = True, edge_dim = None, use_norm=False, 
                 affine = True, is_zinc = False):
        '''
            emb_dim (int): node embedding dimensionality
        '''
        ...
        if self.use_norm:
            self.norm = nn.BatchNorm1d(emb_dim, affine = affine)

    def forward(self, batch):
        x, edge_index, edge_attr = batch.x, batch.edge_index, batch.edge_attr
        x_in = x
        edge_embedding = self.edge_embedding(edge_attr)
        batch.x = self.mlp((1 + self.eps) *x + self.propagate(edge_index, x=x, edge_attr=edge_embedding))
        if self.use_norm:
...
class GINEConvLayer(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dropout, residual):
        ...
        gin_nn = nn.Sequential(
            pyg_nn.Linear(dim_in, dim_in *2), nn.ReLU(),
            pyg_nn.Linear(dim_in*2, dim_out))
...
@register_layer('gine2conv')
class GINE2ConvGraphGymLayer(nn.Module):
    ...
    def __init__(self, layer_config: LayerConfig, **kwargs):
        ...
        gin_nn = nn.Sequential(
            ...
            Linear_pyg(2 * layer_config.dim_out, layer_config.dim_out))
        self.model = pyg_nn.GINEConv(gin_nn)
